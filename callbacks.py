...
class Tensorboard(tf.keras.callbacks.Callback):
    def __init__(self,
        ...
        self.training_writer = tf.summary.FileWriter(os.path.join(self.log_dir, 'training'))
        self.val_writer = tf.summary.FileWriter(os.path.join(self.log_dir, 'val'))
        self.sess = K.get_session()
        self.summary_op = summary_op
        self.epoch = 0
        self.steps_per_epoch = size_dataset // batch_size
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        if batch % self.batch_interval == 0:
            ...
            # print(summary_value)
            self.training_writer.add_summary(summary_value, self.step)
...
class CustomCheckpointer(tf.keras.callbacks.Callback):
    def __init__(self, filepath, custom_model, monitor, mode, save_best_only, save_model=False,
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not self.save_best_only or self.monitor_op(current, self.best):
            ...
            self.best = current
            cur_file_path = os.path.join(self.filepath, '{}-ep{:02d}-End-loss{:.5f}.h5'.format(self.prefix, self.epoch,
                                                                                   np.mean(current)))
            if self.save_model:
                self.custom_model.save('model.h5')
            else:
                self.custom_model.save_weights(cur_file_path, overwrite=False)
